...
FEFE_Multiplayer is a Unity 6 multiplayer boss-fight game targeting ~5 players per session.
Networking uses Unity Netcode for GameObjects (NGO). Human characters are driven by Animancer
(rule-based blend mixers); the dragon is driven by Mecanim (Animator Controllers + blend trees).
The project is in pre-alpha Phase 0 — systems are still being shaped, refactors are routine,
and clean reverts to a prior commit are an accepted recovery path.
...
- For **gameplay design** — setting, roles, win conditions, The Fortress scope, Phase A/B plan: see `design/FEFE_Design.md`.
- For **forward-looking NPC & scale architecture** (active-zone streaming, agent ceilings, NGO-safe NPC patterns for the 300+ agent crowd system): see `design/FEFE_NPC_Architecture.md`.
