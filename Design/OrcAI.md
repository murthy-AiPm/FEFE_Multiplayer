...
Leash-boundary update (2026-05-13): visible targets outside `pursueRadiusFromHome` no longer make an orc flicker between chase and return. `OrcAI` now has a `LeashThreat` substate: the orc can chase a visible target until the orc itself reaches the end of its pursue radius, then it stops pathing, faces the threat, and can hold an Inspector-configured taunt bool such as `IsTaunting` while randomly selecting a taunt blend-tree variant through an int/float parameter such as `TauntIndex`. `leashThreatStatePath` identifies the taunt blend-tree state so the AI can restart each chosen taunt from time 0 and only choose the next variant after the current taunt state reaches normalized time 1. `leashThreatMaxDuration` caps how long the orc keeps taunting a still-visible out-of-range target before returning home. If either the target or the orc is back inside the pursue radius, combat resumes. If the target breaks LoS for `loseSightGraceTime`, the orc returns to normal return/patrol behavior.

Leash harassment update (2026-05-13): if a player successfully damages an orc while it is already in `LeashThreat`, the orc treats that as harassment and ignores home-distance pursue checks entirely until health is at or below `lowHealthPursueRevertRatio`. At that low-health point, the AI reverts to normal leash rules so it can return home instead of remaining exploitable at the boundary. The visible home-distance tuning remains the authored `pursueRadiusFromHome`; the low-health cutoff is under the Leash Threat header.

Low-health return locomotion update (2026-05-13): when the orc disengages into a low-health home return, it ignores ranged alerts until it reaches `OriginalPosition`, the point where it started the scene/game. While health is at or below `lowHealthPursueRevertRatio`, `OrcAI` drives the Animator `Speed` parameter to `-1` so the locomotion blend tree can play the damaged-walk clip, and the NavMeshAgent uses `damagedWalkSpeed` from the Movement header. If health regenerates above the threshold before reaching home, the same return continues but switches to run locomotion/speed. Nearby visible melee targets can interrupt the retreat into a defensive in-place fight, but the orc will not pursue them; if they step out of melee defense range, it resumes returning home. Normal target acquisition resumes after arrival.
...
- If the shooter is visible inside `investigateRadiusFromHome`, the orc immediately targets the shooter. Normal pursue/leash rules decide whether it chases, taunts at the boundary, or later returns.
...
- Ranged-source investigation uses run movement. If a long-range/bounded investigation spots a visible player outside `investigateRadiusFromHome`, that visible player becomes the target and the same pursue/leash/taunt rules apply. If it spots nobody, the return home remains a normal walk.
...
- If a bounded ranged investigation reaches the far boundary and the orc visually confirms a player beyond `investigateRadiusFromHome` while also being outside its home pursue radius, the behavior hands off to `LeashThreat` instead of immediately alert-returning. This lets a visible far-away harasser trigger the same boundary taunt behavior after the orc has actually investigated toward them.
- While running home to alert camp, missed-arrow impacts are ignored so the orc does not oscillate between investigating and returning. A direct hit can interrupt alert-return if the shooter is visible; normal pursue/leash rules handle distance after that.
- Alert-return can still reacquire visible players; normal pursue/leash rules decide whether that becomes chase or leash-boundary taunt.
